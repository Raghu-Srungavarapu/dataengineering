...
def getSQLcmnds(rowlist):
	...
	for row in rowlist:
		...
		cmdlist.append(cmd)
	cmdlist.append(f"INSERT INTO {TableName} SELECT * FROM {temptable};")
	return cmdlist
...
def createTempTable(conn):
	...
	with conn.cursor() as cursor:
		...
		print(f"Created Temporary table {temptable}")
...
def load(conn, icmdlist):
	...
	with conn.cursor() as cursor:
		...
		with open(Datafile, 'r') as f:
			next(f)
			cursor.copy_from(f, temptable, sep=',', null='')
			cursor.execute(f"AlTER TABLE {temptable} add Year Integer DEFAULT {Year}")
			cursor.execute(f"INSERT INTO {TableName}(CensusTract,State,County,TotalPop, Men, Women, Hispanic, White ,Black,Native,Asian,Pacific,Citizen,Income,IncomeErr,IncomePerCap,IncomePerCapErr,Poverty,ChildPoverty,Professional,Service,Office,Construction,Production,Drive,Carpool,Transit,Walk,OtherTransp, WorkAtHome, MeanCommute, Employed, PrivateWork, PublicWork, SelfEmployed, FamilyWork, Unemployment, Year) SELECT * FROM {temptable};")
			conn.commit()
